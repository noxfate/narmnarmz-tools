...
def get_01header_SLWBEZ_by_key(dataWb, keyDict):
    ws = dataWb.get_sheet_by_name("Task List Header")
    keys = dict(keyDict)
    keys.pop("VORNR")
    keys.pop("MERKNR")
...
def validate(wb, dataWb):
    ...
    DATA_TAB_NAME = "MIC" # sheet name to find data
    ...
    active_ws = wb.get_sheet_by_name("5. Inspection Characteristic")
    if (IS_FREEZE):
    ...
    for i in range(DATA_ROW_COUNT+1, n_of_data + DATA_ROW_COUNT+1):
        ...
        opr_ws = dataWb.get_sheet_by_name("TaskList Operation")
        d = dict()
        d["PLNNR"] = PLNNR
        d["PLNAL"] = PLNAL
        d["VORNR"] = VORNR
